...
# Analysis of multiple assay end-points

```{r}
dose_response <- geo_tox_data$dose_response |> 
  filter(call == "Active")
casn <- sort(unique(dose_response$casn))
...
suppressWarnings(
  geoTox <- GeoTox() |> 
    ...
    calculate_response(hill_params = fit_hill(split(dose_response, ~casn + endp, drop = TRUE))) |> 
...
plot(geoTox, quantiles = 0.975, quantile_labels = "upper 95th percentile")
