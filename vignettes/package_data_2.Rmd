...
```{r, eval=FALSE}
cHTS <- read_tsv("cHTS2023_invitrodb35_20231006.txt",
...
cHTS_hits_TSV <- cHTS |> 
  filter(Assay %in% assays,
         Endpoint == "Call",
         Response == "Active") |> 
  distinct(Assay, CASRN, DTXSID, ChemicalName)
```

> **NOTE:** There are some discrepancies between the API and downloaded file.

```{r, echo=FALSE}
load("~/dev/GeoTox/data-new/cHTS_hits.RData")
```
```{r}
all.equal(cHTS_hits_API |> arrange(assay, casrn),
          cHTS_hits_TSV |> arrange(Assay, CASRN),
