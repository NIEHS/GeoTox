---
title: "GeoTox Package Data 2"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{GeoTox Package Data 2}
...
library(httk, warn.conflicts = FALSE)
library(readxl)
library(httr2)
library(qs)
...
```{r, eval=TRUE}
exposure <- read_xlsx(paste0(base_path,"/data-raw/2019_Toxics_Ambient_Concentrations.xlsx"))
...
exposure_casrn <- read_csv(paste0(base_path,"/data-raw/CCD-Batch-Search.csv"),
...
# Add CASRN to geo_tox_data object
...
geo_tox_data$dose_response <- get_ICE_dose_resp(assays = assays,
                                                chemids = chemids)
```
...
age <- read_csv(paste0(base_path,"/data-raw/cc-est2019-alldata-37.csv"),
                show_col_types = FALSE)
...
```{r, eval=TRUE}
places <- read_csv(paste0(base_path,"/data-raw/PLACES__County_Data__GIS_Friendly_Format___2020_release.csv"),
                   show_col_types = FALSE)
...
> **NOTE:** The number of samples in this example dataset is very small and should be increased if using this method for your analysis.
...
```{r, eval=TRUE}
county <- st_read(paste0(base_path,"/data-raw/cb_2019_us_county_5m.shp"))
state <- st_read(paste0(base_path,"/data-raw/cb_2019_us_state_5m.shp"))
...
```
# Save the data
```{r save the data, eval=TRUE}

save(geo_tox_data, file = paste0(base_path,"/data/geo_tox_data_manuscript.RData"))
