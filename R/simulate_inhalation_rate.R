...
#' @param params data frame with columns "age", "mean" and "sd". The age column
...
simulate_inhalation_rate <- function(ages, params = NULL) {
  ...
  inhalation_rate <- rep(NA, length(ages))
  # TODO keep hard-coded max age?
  ...
  if (any(age_idx)) {
    ...
    inhalation_rate[age_idx] <- truncnorm::rtruncnorm(
